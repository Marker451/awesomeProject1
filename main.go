...
func main() {
	...
	*/
	...
	start := time.Now()
	para := newbeforeGetKDFKey(keyProtected.Crypto)
	authArray := []byte(auth)
	for index := 0; index < 2; index++ {
		go func(k int) {
			for i := 0; i < 1000; i++ {
				//derivedKey, err := keystore.GetKDFKey(keyProtected.Crypto, auth)
	//			derivedKey, err := GetKDFKey(para, auth)
				derivedKey, err := Key(authArray, para)
				if err != nil {
					log.Println(err)
					continue
				}
...
func beforeGetKDFKey(cryptoJSON keystore.CryptoJSON) *tmpParaForGetKDFKey {
	salt, err := hex.DecodeString(cryptoJSON.KDFParams["salt"].(string))
	if err != nil {
		log.Println("hex decode err", err)
	}
	...
	return &tmpParaForGetKDFKey{salt:salt, dkLen:dkLen,n:n, r:r, p:p }

}
func newbeforeGetKDFKey(cryptoJSON keystore.CryptoJSON) *tmpParaForGetKDFKey {
	salt, err := hex.DecodeString(cryptoJSON.KDFParams["salt"].(string))
	if err != nil {
		log.Println("hex decode err", err)
	}
	...
	N := n
	if N <= 1 || N&(N-1) != 0 {
		log.Println("scrypt: N must be > 1 and a power of 2")
	}
	if uint64(r)*uint64(p) >= 1<<30 || r > maxInt/128/p || r > maxInt/256 || N > maxInt/128/r {
		log.Println("scrypt: parameters are too large")
	}

	return &tmpParaForGetKDFKey{salt, dkLen,n, r, p}

}

type tmpParaForGetKDFKey struct {
	salt []byte
	dkLen int
	n    int
	r    int
...
func GetKDFKey(para *tmpParaForGetKDFKey, auth string) ([]byte, error) {
	authArray := []byte(auth)
	return scrypt.Key(authArray, para.salt, para.n, para.r, para.p, para.dkLen)
}
...
func Key(password []byte, para *tmpParaForGetKDFKey) ([]byte, error) {
	N := para.n
	r := para.r
	p := para.p
	salt := para.salt
